...
import type { CatalogIntakeDraft, CatalogIntakePipelineView, CatalogIntakeQualityProfile } from '../api/types';
...
function displayInferenceMode(mode: CatalogIntakeQualityProfile['capacity_proposal']['inference_mode']) {
  if (mode === 'remote-endpoint') return 'Remote endpoint';
...
function QualityReview({ quality }: { quality: CatalogIntakeQualityProfile }) {
  const artifacts = Object.values(quality.artifacts);
  const presentArtifacts = artifacts.filter((artifact) => artifact.status === 'present' || artifact.status === 'present-valid').length;
  const security = quality.security_summary;
  return <Panel title="Quickstart quality review">
    <div className="flex flex-wrap items-center gap-3">
      <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${quality.gate.status === 'blocked' ? 'bg-[#4A1F1F] text-[#FF8D85]' : 'bg-[#2B2414] text-[#F8C95E]'}`}>
        {quality.gate.status === 'blocked' ? 'Blocked' : 'Review required'}
      </span>
      <span className="text-sm text-[#A3A3A3]">Human review required</span>
    </div>
    <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      <div className="rounded border border-[#333] bg-[#1A1A1A] p-3"><p className="text-xs text-[#A3A3A3]">Authoring artifacts</p><p className="mt-1 font-semibold text-white">{presentArtifacts} of {artifacts.length} present</p></div>
      <div className="rounded border border-[#333] bg-[#1A1A1A] p-3"><p className="text-xs text-[#A3A3A3]">Showroom</p><p className="mt-1 font-semibold text-white">{countLabel(quality.showroom.hands_on_module_count, 'hands-on module')}</p></div>
      <div className="rounded border border-[#333] bg-[#1A1A1A] p-3"><p className="text-xs text-[#A3A3A3]">Inference signal</p><p className="mt-1 font-semibold text-white">{displayInferenceMode(quality.capacity_proposal.inference_mode)}</p></div>
      <div className="rounded border border-[#333] bg-[#1A1A1A] p-3"><p className="text-xs text-[#A3A3A3]">Supply chain</p><p className="mt-1 font-semibold text-white">{countLabel(security.mutable_image_count, 'mutable image')}</p></div>
    </div>
    <div className="mt-4 grid gap-4 lg:grid-cols-2">
      <div>
        <h3 className="text-sm font-semibold text-white">Activation findings</h3>
        {quality.gate.blocking_findings.length ? <ul className="mt-2 space-y-2">{quality.gate.blocking_findings.map((finding) => <li key={finding} className="rounded border border-[#C9190B]/30 bg-[#2B1717] p-3 text-sm text-[#FF8D85]">{finding}</li>)}</ul> : <p className="mt-2 text-sm text-[#F8C95E]">No static blockers; certification evidence is still required.</p>}
      </div>
      <div>
        <h3 className="text-sm font-semibold text-white">Safety boundary</h3>
        <p className="mt-2 rounded border border-[#333] bg-[#1A1A1A] p-3 text-sm text-[#D2D2D2]"><strong className="text-white">Analysis only.</strong> This review cannot modify source, publish a catalog item, provision a lab, certify a release, or relax measured capacity gates.</p>
        <p className="mt-2 text-xs text-[#A3A3A3]">Portfolio overlap: {quality.portfolio_overlap.status}. A pinned inventory is required before comparison.</p>
      </div>
    </div>
  </Panel>;
}
...
export default function CatalogIntakeDetail() {
  ...
  const quality = draft.catalog_preview?.metadata.intake_quality;
  ...
  return <div className="mx-auto max-w-7xl px-6 py-10 lg:px-8">
    ...
    <div className="mt-7 grid gap-6 lg:grid-cols-3"><div className="space-y-6 lg:col-span-2">
      ...
      {quality && <QualityReview quality={quality} />}
      ...
      <Panel title="Certification gates">
        {pipeline ? <ol className="space-y-3">{pipeline.gates.map((gate) => <li key={gate.gate_id} className="rounded border border-[#333] bg-[#1A1A1A] p-4">
          <div className="flex flex-wrap items-center justify-between gap-2"><strong className="text-sm text-white">{gate.label}</strong><span className={`text-xs font-semibold uppercase ${gate.status === 'passed' ? 'text-[#7CE2B2]' : 'text-[#F8C95E]'}`}>{gate.status}</span></div>
          <p className="mt-2 text-xs text-[#A3A3A3]">Requires: {gate.required_evidence.join(', ')}</p>
          {gate.blockers.length > 0 && <ul className="mt-2 list-inside list-disc space-y-1 text-xs text-[#F8C95E]">{gate.blockers.map((blocker) => <li key={blocker}>{blocker}</li>)}</ul>}
        </li>)}</ol> : <Empty>Gate contract unavailable</Empty>}
      </Panel>

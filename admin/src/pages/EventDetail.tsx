  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-10 lg:px-8">
      ...
      <section className="mt-8 rounded border border-[#333] bg-[#212121]">
        ...
        <p className="border-t border-[#333] px-5 py-3 text-xs text-[#A3A3A3]">{forecast.explanation} Evidence {forecast.evidence_matches ? 'matches' : 'has drifted'}; model serving evidence: {forecast.model_health_status ?? 'not evaluated'}{forecast.model_health_snapshot_id ? ` (${forecast.model_health_snapshot_id.slice(0, 19)}…)` : ''}. {forecast.current_active_reservations} fleet reservations currently consume certified capacity. {capacity.dr_reserved_capacity} DR-reserved and {capacity.uncertified_capacity} uncertified seats are excluded. Artifact cold-pull and measured in-flight usage remain separate gates.</p>
      </section>
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-2">
        ...
        <section className="rounded border border-[#333] bg-[#212121] p-5"><h2 className="text-lg font-semibold text-white">Event contract</h2><dl className="mt-4 grid grid-cols-2 gap-4 text-sm"><div><dt className="text-[#A3A3A3]">Cohorts</dt><dd className="text-white">{manifest.cohorts.length}</dd></div><div><dt className="text-[#A3A3A3]">Labs</dt><dd className="text-white">{manifest.labs.length}</dd></div><div><dt className="text-[#A3A3A3]">Exposure</dt><dd className="text-white">{label(manifest.exposure_policy)}</dd></div><div><dt className="text-[#A3A3A3]">Retention</dt><dd className="text-white">{manifest.retention.hours} hours</dd></div><div><dt className="text-[#A3A3A3]">Owner</dt><dd className="text-white">{manifest.owner}</dd></div><div><dt className="text-[#A3A3A3]">Approver</dt><dd className="text-white">{manifest.technical_approver}</dd></div></dl><div className="mt-4 border-t border-[#333] pt-4"><h3 className="text-xs font-semibold uppercase tracking-wide text-[#A3A3A3]">Declared model requirements</h3>{manifest.labs.map((lab) => <p key={lab.lab_ref} className="mt-2 text-xs text-[#D2D2D2]"><span className="font-semibold text-white">{lab.catalog_id}</span>: {lab.required_models?.length ? lab.required_models.join(' · ') : 'No model IDs declared'}</p>)}<p className="mt-3 text-xs text-[#A3A3A3]">Certified model IDs are placement evidence, not a live inference health check.</p></div></section>
      </div>
    </div>
  );
}

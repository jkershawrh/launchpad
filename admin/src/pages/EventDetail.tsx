...
export default function EventDetail() {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([api.getEvent(eventId), api.getEventStatus(eventId)])
      .then(([eventRecord, statusRecord]) => { if (active) { setEvent(eventRecord); setStatus(statusRecord); } })
  ...
  if (!event || !status) return <div className="mx-auto max-w-7xl px-6 py-10 lg:px-8"><p role="status" className="rounded border border-[#333] bg-[#212121] p-6 text-[#A3A3A3]">Loading event status…</p></div>;

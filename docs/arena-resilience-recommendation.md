...
4. Keep workshop provisioning staggered and allow participants to operate concurrently. The durable lifecycle queue now enforces this with `SERIALIZE_WORKSHOP_PROVISIONING=true`: one workshop provision lease may run fleet-wide while later orders remain queued. Reclaim is not blocked. The pilot should not advertise simultaneous rollout mutation as a participant requirement.

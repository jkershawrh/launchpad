...
## Decommissioned intake

`agentops-observability` is deprecated and cannot be ordered. Its historical
certification evidence remains immutable, while its content and workload
sources are retained only until the archive-removal gate proves that no active
catalog or release path consumes them.

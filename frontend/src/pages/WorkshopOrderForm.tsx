...
export default function WorkshopOrderForm({ embedded = false }: { embedded?: boolean }) {
  ...
  return <div className={embedded ? '' : 'mx-auto max-w-2xl px-4 py-10'}>
    ...
    {preview && <section className="mt-6 rounded-md border border-[#333] bg-[#212121] p-6">
      ...
      <div className="mt-5 grid grid-cols-2 gap-4 border-y border-[#333] py-4 text-sm text-[#8A8D90] sm:grid-cols-4"><div><b className="text-xl text-white">{preview.seats_requested}</b><br/>seats</div><div><b className="text-xl text-white">{preview.estimated_resources.cpu_millicores / 1000}</b><br/>CPU cores</div><div><b className="text-xl text-white">{Math.round(preview.estimated_resources.memory_mib / 1024)}</b><br/>GiB memory</div><div><b className="text-xl text-white">{preview.estimated_resources.pods}</b><br/>pod slots</div></div>

...
import { reclaimActionLabel, workshopProgressLabel, workshopReadiness, workshopSeatAccess } from '../workshopOrderContract';
...
export default function WorkshopDetail() {
  ...
  const exportLinks = async () => {
    if (!workshop) return;
    const rows = ['seat,participant,status,access_url', ...workshop.seats.map((seat) => `${seat.seat_number},${seat.participant_id || ''},${seat.status},${workshopSeatAccess(workshop, seat)?.url || ''}`)];
  ...
  return <div className="mx-auto max-w-6xl px-4 py-10 text-white">
    ...
    {workshop.exposure_policy === 'public_code' && workshop.public_url && <section className="mt-5 rounded-lg border border-[#0068B5]/60 bg-[#0068B5]/10 p-5"><p className="text-xs font-semibold uppercase tracking-wide text-[#58A6E7]">Participant access</p><p className="mt-2 text-sm text-[#D2D2D2]">Give every participant this shared workshop URL. It keeps Showroom and declared lab tools behind the Launchpad access gateway.</p><a className="mt-4 inline-block rounded bg-[#EE0000] px-4 py-2 text-sm font-semibold text-white" href={workshop.public_url} target="_blank" rel="noreferrer">Open participant portal</a><p className="mt-3 break-all font-mono text-xs text-[#B8BBBE]">{workshop.public_url}</p></section>}
    ...
    <div className="mt-6 overflow-x-auto rounded-lg border border-[#333] bg-[#212121]"><table className="w-full text-left text-sm"><thead className="bg-[#151515] text-[#B8BBBE]"><tr><th className="p-3">Seat</th><th className="p-3">Participant</th><th className="p-3">Status</th><th className="p-3">Access</th><th className="p-3">Error</th></tr></thead><tbody>{workshop.seats.map((seat) => { const access = workshopSeatAccess(workshop, seat); return <tr key={seat.seat_id} className="border-t border-[#333]"><td className="p-3 font-semibold">{seat.seat_number}</td><td className="p-3 font-mono text-xs text-[#B8BBBE]">{seat.participant_id}</td><td className="p-3">{seat.status}</td><td className="p-3">{access ? <a className="font-semibold text-[#58A6E7]" href={access.url} target="_blank" rel="noreferrer">{access.label}</a> : '—'}</td><td className="max-w-xs p-3 text-xs text-red-300">{seat.error || '—'}</td></tr>; })}</tbody></table></div>
  </div>;
}

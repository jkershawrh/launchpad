    ...
    <div className="bg-white rounded-lg border p-6 mb-6">
      ...
      <div className="grid md:grid-cols-3 gap-6">
          ...
          {wp && (
            <dl className="space-y-1 text-xs">
              <div className="flex justify-between">
                <dt className="text-[#6A6E73]">Compute</dt>
                <dd className="text-[#151515] font-medium">{wp.compute_intensity}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-[#6A6E73]">Memory</dt>
                <dd className="text-[#151515] font-medium">{wp.memory_intensity}</dd>
              </div>
              <div className="flex justify-between">
                <dt className="text-[#6A6E73]">I/O Pattern</dt>
                <dd className="text-[#151515] font-medium">{wp.io_pattern}</dd>
              </div>
            </dl>
          )}
        </div>
        ...
        <div>
          ...
          <p className="text-xs text-[#6A6E73] mb-3">Quota: {decision.recommended_quota}</p>
          {decision.fallback_chain.length > 0 && (
            <div>
              <p className="text-xs text-[#6A6E73] mb-1">Fallback chain:</p>
              <div className="flex flex-wrap gap-1">
                {decision.fallback_chain.map((c) => (
                  <span key={c} className="text-xs bg-[#F0F0F0] text-[#151515] px-2 py-0.5 rounded">{c}</span>
                ))}
              </div>
            </div>
          )}
